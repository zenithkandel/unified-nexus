...
try {
    ...
    $stmt = $pdo->prepare('UPDATE clubs SET display_order = :order WHERE id = :id');
    ...
    $pdo->commit();
    json_success('Clubs reordered successfully.');
} catch (Throwable $e) {
    if (isset($pdo) && $pdo->inTransaction()) {
        $pdo->rollBack();
    }
    json_error('Failed to reorder clubs.', [$e->getMessage()], 500);
}
