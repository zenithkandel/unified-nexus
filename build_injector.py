...
js_code = \"\"\"(function () {
  ...
  style.textContent = \
    .theme-grid { display: grid; grid-template-columns: repeat(5, 1fr); gap: 8px; padding: 12px; overflow-y: auto; overflow-x: hidden; }
  ...
  function createTheme(name, filterGroup, h, bgS, bgL, txtL, accS, accL) {
    const isDarkBg = bgL < 50;
    const bg1 = hslToHex(h, bgS, bgL);
    const bg2 = hslToHex(h, bgS, max0(bgL - 5));
    const bg3 = hslToHex(h, bgS, max0(bgL - 10)); // darkest element
    const bg4 = hslToHex(h, bgS, min100(bgL + 15)); // muted items
    
    const textBright = hslToHex(h, min100(bgS/2), isDarkBg ? txtL : 100 - txtL);
    const textMuted  = hslToHex(h, bgS, isDarkBg ? txtL - 25 : 100 - (txtL - 25));
    ...
    themes.push({
      ...
      vars: {
        ...
        '--color-cream': bg3,     // Deepest accent box
        '--c-661B28': bg3,
  ...
  function renderGrid(filter) {
    ...
    themes.forEach(t => {
      ...
      item.style.background = \linear-gradient(135deg, \ 50%, \ 50%)\;
      
      item.onclick = () => {
        Object.keys(t.vars).forEach(k => {
          document.documentElement.style.setProperty(k, t.vars[k]);
        });
        console.log(\[Injector] Applied theme: \\);
      };
      
      grid.appendChild(item);
      themeNodes.push(item);
    });
  }
  ...
  console.log('[Theme Injector] Overhauled. Drag panel, switch modes, live preview dynamically generated spectrum.');
})();\"\"\"

with open('js/injector.js', 'w', encoding='utf-8') as f:
    f.write(js_code)

print(\"Injector Overhauled.\")